...
//auto recording all
    if (autorecord === true) { if (command) await conn.sendPresenceUpdate('recording', m.chat) }
//autotyper all
  if (autotyping === true) { if (command) await conn.sendPresenceUpdate('composing', m.chat) }
      //Mute Chat\\
...
let ytta = text.split('|')[1]
if (!nonya) throw `Nomornya? Ex: ${prefix + command} 628xx|Halo banh`
if (!ytta) throw `Teksnya? Ex: ${prefix + command} 628xx|Halo`
conn.sendMessage(nonya + '@s.whatsapp.net', { text: ytta })
}
break
case 'setcmd': {
...
await conn.groupAcceptInvite(result).then((res) => reply(`Succes banh`)).catch((err) => reply(`Error`))
}
break
case 'leave': {
if (!isCreator) throw mess.owner
